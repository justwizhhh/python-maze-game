...
class GameMap:
    ...
    def is_valid_tile(self, layer_id: int, check_tile: Tuple[int, int]) -> bool:
        try:
            tile = self.map[layer_id][1][int(check_tile[1])][int(check_tile[0])]
            if tile:
                return True
            else:
                return False
    ...
    def get_empty_tiles(self) -> list[tuple[int, int]]:
        """Finds all empty tiles in the currently loaded map."""
        empty_tiles = []
        for layer in self.map:
            for row_index, row in enumerate(layer[1]):
                for col_index, tile in enumerate(row):
                    if not tile:  # Check if the tile is empty
